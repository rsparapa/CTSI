...
\begin{frame}[fragile]\frametitle{\bf\textcolor{blue}{Medi-Span and RxNorm
...
\begin{verbatim}
medispan=dbGetQuery(db, "select distinct pharm_class, 
                         pharm_subclass,
                         ingredient_rxcui_name
                         from fh_hb_mar_table_jupyter 
                         order by pharm_class, 
                         pharm_subclass,
                         ingredient_rxcui_name")
write.csv(medispan, "medispan.csv", row.names=FALSE)
\end{verbatim}
\end{frame}
