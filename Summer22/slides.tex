...
\begin{itemize}
\item  
\textcolor{PineGreen}{[\href{https://degauss.org}{DeGAUSS: Decentralized Geomarker Assessment for Multi-Site Studies}]}
\item ``Patient Demographics'' \texttt{geocode\_result}
\item Self-explanatory: \texttt{po\_box} and \texttt{non\_address\_text} 
...
designates block groups without an ADI due to key
  missing variables from the American
...
plot(density(b$adi, from=1, to=100, na.rm=TRUE), 
     xlab='ADI', ylab='CDF', main='1: best, 100:worst')
abline(h=0.01, col=8)
\end{verbatim}
\end{frame}
